function [rst, OA, AA, AR, accuracies] = Classification_SVM(im, gt, emap, pct, indexes)
...
% --------------
%
% Classification_SVM classifies a given image using the SVM classifier
...
% learn the model
model = svmtrain(Y,X,'-s 0 -t 1 -q 1');

% Predicts the labels of the given data based on the model learnt by k-NN
rst = svmpredict(reshape(gt,h*w,1),to2d(im)',model,'-q 1');
rst = to3d(rst',w,h);
