function [Z, T] = caLFDA(X,Y,r,optimisation,MAX_PER_CLASS,metric,kNN)
%
...
%    Z: r x n matrix of reduced samples
%
...
if(optimisation == 1)
    maxClasses = max(Y) + 1;
    classes(maxClasses+1)=0;
    for i=1:n
        if size(classes,2)<(Y(i)+1) || classes(Y(i)+1) == 0
            classes(Y(i)+1) = 1;
            newLabel(Y(i)+1) = Y(i);
        else
            if classes(newLabel(Y(i)+1)+1) == MAX_PER_CLASS
                newLabel(Y(i)+1) = newLabel(Y(i)+1) + maxClasses;
                classes(newLabel(Y(i)+1)+1) = 1;
            else
                classes(newLabel(Y(i)+1)+1) = classes(newLabel(Y(i)+1)+1) + 1;
            end
            Y(i) = newLabel(Y(i)+1);
        end
    end
